...
class LPGEngine(PDDLPlanner):
    ...
    def _plan_from_file(self, problem: 'up.model.Problem', plan_filename: str, get_item_named: Callable[[str],
        ...
        with open(plan_filename) as plan:
            for line_num, line in enumerate(plan.readlines(), 1):
                # Skip empty lines and comments
                if re.match(r'^\s*(;.*)?$', line):
                    continue
                
                # Check for "no solution"
                if re.match(r'no solution', line.lower()):
                    return None
                
                # Clean the line - remove comments and strip
                clean_line = line.split(';;')[0].strip()
                
                # More flexible regex pattern for temporal plans
                # Matches: "0.0000:   (action param1 param2)  [1.0000]"
                # or: "0:   (action param1 param2)  [1]"
                res = re.match(r'^([\d.]+)\s*:\s*\(\s*([\w?-]+)((?:\s+[\w?-]+)*)\s*\)\s*\[\s*([\d.]+)\s*\]', clean_line.lower())
                
                if res:
                    action = get_item_named(res.group(2))
                    parameters = []
                    param_str = res.group(3).strip()
                    if param_str:
                    ...
                    if tt:
                        start = Fraction(res.group(1))
                        dur = Fraction(res.group(4))
                    ...
                    # If line is not empty and doesn't match, it might be a header or metadata
                    if clean_line and not any(x in clean_line.lower() for x in ['time:', 'solution', 'plan', 'metric']):
                        raise UPException(f'Error parsing plan line {line_num}: {repr(line)}\nCleaned: {repr(clean_line)}')
